...
export default function Stream() {
  ...
  const handleNegotiationNeededEvent = async (peer) => {
    ...
    await fetchFunction(peer.localDescription, peer);
    // console.log(offer);
  };
  const fetchFunction = async (sdp, peer) => {
    try {
      //http://sfuvideo.herokuapp.com/broadcast
      //http://localhost:9000/broadcast
      const response = await fetch("http://sfuvideo.herokuapp.com/broadcast", {
        method: "POST",
        body: JSON.stringify({
          sdp,
        }),
      ...
      const desc = new RTCSessionDescription(result.sdp);
  ...
  const iceFetch = async (ice, peer) => {
    const response2 = await fetch("http://sfuvideo.herokuapp.com/addIce", {
      method: "POST",
      body: JSON.stringify({
        ice: ice,
      }),
